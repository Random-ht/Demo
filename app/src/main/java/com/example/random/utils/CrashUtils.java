 ...
 *     desc  : 崩溃相关工具类
 *
 *     以下是需要在Application里面的 onCreate()  方法里面注册
 *     初始化异常捕获的上下文关联    Utils.init(AppApplication.this);
 *     初始化异常捕获工具类          CrashUtils.init();
 *     错误日志   保存在  Android-->> data-->> 包名-->> cache-->> crash  下      错误日志的命名规则为 ：系统时间   11-11 11-11-11.txt  即11月11号，11点11分11秒
 *
 * </pre>
 */
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            //这里写入的是设备信息之类的东西   这个在其他地方单独处理了  不用写入到文件中了
//                            pw.write(CRASH_HEAD);
